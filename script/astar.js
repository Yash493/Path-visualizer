var Data;
var Queue = []; //Open queue
var visited = []; //close queue
var found = false;
var totalPath = [];
...
function calculateHeuristic(Data, startNode, endNode) {
	...
	for (let i = 0; i < Data.length; i++) {
		for (let j = 0; j < Data.length; j++) {
			Data[i][j].heuristic =
				Math.abs(Data[i][j].i - endNode.i) + Math.abs(Data[i][j].j - endNode.j);
		}
	}
}
	...
	while (Queue.length != 0) {
		...
		for (let i = 0; i < current.neighbors.length; i++) {
			...
			if (f < current.neighbors[i].distance) {
				...
				current.neighbors[i].function =
					current.neighbors[i].distance + current.neighbors[i].heuristic;
...
function AstarPath(path, frames, nodes, speed) {
	...
	setTimeout(function () {
		...
		foundStatus.innerHTML = "Path found !!";
		pathStatus.innerHTML =
			`Optimal path : ` +
			path.length +
			`<br/>Node visited after searching ` +
			nodes +
			` nodes.`;
	}, (++frames + 2) * 20 * speed);
}
